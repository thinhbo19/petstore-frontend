...
import { getAllBookingUser } from "@/src/services/apiBooking";
...
export default function BookingHistoryPage() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      if (userId) {
        try {
          const res = await getAllBookingUser(userId, accessToken);
  ...
  return (
    ...
    >
      ...
      >
        {["All", "Processing", "Confirmed", "Completed", "Cancelled"].map(
          (status) => (
            <Button
              key={status}
              onClick={() => {
            ...
            >
              ...
              >
                <Grid item xs={12} md={8}>
                  ...
                  >
                    Type Service: {order.services[0].type}
                  </Typography>
                  <Typography
                    sx={{
                      fontSize: { xs: "0.8rem", sm: "1rem", md: "1.2rem" },
                  ...
                  >
                    Booking Date: {formatDate(order.bookingDate)}
                  </Typography>
                </Grid>
                <Grid
                  item
                ...
                >
                  <Button
                    ...
                    onClick={() => router.push(`/booking-detail/${order._id}`)}
                  >
                    See Details
                  </Button>
                </Grid>
