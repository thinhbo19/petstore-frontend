...
const SpeciesAdminComp = () => {
  ...
  const handleDelete = async (id) => {
    setLoading(true);
    try {
      await axios.delete(`${apiUrlSpecies}/${id}`, {
        headers: {
          Authorization: `Bearer ${accessToken}`,
        },
      });
  ...
  const filteredDataList = dataList.filter((item) =>
    item.nameSpecies.toLowerCase().includes(searchTerm.toLowerCase())
  );
  ...
  return (
    <div style={{ padding: "20px" }}>
      <div style={{ marginBottom: "20px" }}>
        <h2 style={{ textTransform: "uppercase" }}>Species Management</h2>
      </div>
      <SearchBar
...
export default SpeciesAdminComp;
