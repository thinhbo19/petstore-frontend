...
import { apiUrlCategory } from "@/src/services/config";
...
const CategoryAdminComp = () => {
  ...
  const handleDeleteAll = async () => {
    ...
    if (confirmation.isConfirmed) {
      ...
      try {
        await Promise.all(
          selectedIds.map((id) =>
            axios.delete(`${apiUrlCategory}/${id}`, {
              headers: {
  ...
  const handleDelete = async (id) => {
    setLoading(true);
    try {
      await axios.delete(`${apiUrlCategory}/${id}`, {
        headers: {
          Authorization: `Bearer ${accessToken}`,
        },
      });
  ...
  return (
    <div style={{ padding: "20px" }}>
      <div style={{ marginBottom: "20px" }}>
        <h2 style={{ textTransform: "uppercase" }}>Categories Management</h2>
      </div>
      <SearchBar
...
export default CategoryAdminComp;
